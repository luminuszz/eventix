...
export class OnTicketCreatedEventHandler implements IEventHandler<TicketCreatedEvent> {
  private logger = new Logger(OnTicketCreatedEventHandler.name)

  handle(event: TicketCreatedEvent) {
    this.logger.log(`[OnTicketCreatedEventHandler] ${event} -> sending email with new ticket`)
  }
}
